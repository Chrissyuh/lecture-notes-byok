...
class LectureNotesDb extends Dexie {
  ...
  cardReviews!: Table<FlashcardReview, string>
  ...
  constructor() {
    ...
    this.version(4).stores({
      ...
      segments: 'id, lectureId, index',
      notes: 'id, lectureId, createdAt',
      cardReviews: 'id, lectureId, noteId, cardId, lastReviewedAt',
      materials: 'id, lectureId, kind, createdAt',
      jobs: 'id, lectureId, type, status, runAfter, targetId',
      providers: 'id',
      settings: 'id',
...
export async function deleteLectureCascade(lectureId: string) {
  await db.transaction('rw', [db.lectures, db.chunks, db.segments, db.notes, db.cardReviews, db.materials, db.jobs], async () => {
    ...
    await db.cardReviews.where('lectureId').equals(lectureId).delete()
...
export async function getLocalDataStats(): Promise<LocalDataStats> {
  const [lectures, audioChunks, transcriptSegments, notes, reviewedCards, materials, queuedJobs, chunks, materialRows] = await Promise.all([
    db.lectures.count(),
    db.chunks.count(),
    db.segments.count(),
    db.notes.count(),
    db.cardReviews.count(),
    db.materials.count(),
